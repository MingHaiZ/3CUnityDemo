...
public class Collectable : MonoBehaviour
{
    ...
    public bool randomizeInitalDirection = true;
    public float collisionRadius = 0.5f;
    public float bounciness = 0.98f;
    public float maxBounceYVelocity = 10f;
    public float minBounceYVelocity = 3f;
    ...
    public bool m_ghosting = true;
    protected float m_elapseGhostingTime;
    protected float m_elapseLeftTime;
    ...
    protected virtual void HandleGhosting()
    {
        if (m_ghosting)
        {
            m_elapseGhostingTime += Time.deltaTime;
            if (m_elapseGhostingTime >= ghostingDuration)
            {
    ...
    protected virtual void HandleLeftTime()
    {
        if (hasLifeTime)
        {
            m_elapseLeftTime += Time.deltaTime;
            if (m_elapseLeftTime >= lifeTimeDuration)
            {
                Vanish();
            }
        }
    ...
    protected virtual void HandleSweep()
    {
                ...
                Physics.DefaultRaycastLayers, QueryTriggerInteraction.Ignore))
        {
            if (!hit.collider.CompareTag(GameTag.Player))
            {
                ...
                if (m_velocity.y <= minBounceYVelocity)
                {
                    usePhysics = false;
                }
            }
